//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "TTTBaseModel.h"

#import "TTTJumpProtocol-Protocol.h"

@class NSArray, NSNumber, NSString, TTTBuryDataModel, TTTChannelViewContoller, TTTMultiTabListModel, TTTTabConfigModel;

@interface TTTTabModel : TTTBaseModel <TTTJumpProtocol>
{
    _Bool _isTabSelect;
    NSString *_groupId;
    NSString *_name;
    NSString *_backgroundColor;
    NSString *_defaultPicUrl;
    NSString *_activePicUrl;
    NSArray *_secondTabList;
    unsigned long long _selectSecondTabIndex;
    NSNumber *_timeRemain;
    NSString *_stageStatus;
    long long _rushState;
    NSString *_topText;
    NSString *_bottomText;
    NSString *_multiParams;
    TTTMultiTabListModel *_multiFloorModel;
    double _nameWidth;
    NSString *_floorTemplate;
    NSString *_tabStyle;
    long long _tabType;
    NSNumber *_width;
    NSNumber *_height;
    NSNumber *_imageWidth50;
    NSNumber *_textWidth;
    double _tabWidth;
    unsigned long long _imageStatus;
    TTTTabConfigModel *_tabConfig;
    TTTChannelViewContoller *_channelVC;
    long long _firstTabIndex;
    TTTBuryDataModel *_buryData;
    TTTBuryDataModel *_pageBuryData;
    TTTBuryDataModel *_rushBuryData;
    TTTBuryDataModel *_groupBuyBuryData;
    struct CGPoint _horizontalContentOffset;
    struct CGPoint _listViewScrollOffset;
}

+ (id)objectClassInArray;
@property(retain, nonatomic) TTTBuryDataModel *groupBuyBuryData; // @synthesize groupBuyBuryData=_groupBuyBuryData;
@property(retain, nonatomic) TTTBuryDataModel *rushBuryData; // @synthesize rushBuryData=_rushBuryData;
@property(retain, nonatomic) TTTBuryDataModel *pageBuryData; // @synthesize pageBuryData=_pageBuryData;
@property(retain, nonatomic) TTTBuryDataModel *buryData; // @synthesize buryData=_buryData;
@property(nonatomic) struct CGPoint listViewScrollOffset; // @synthesize listViewScrollOffset=_listViewScrollOffset;
@property(nonatomic) long long firstTabIndex; // @synthesize firstTabIndex=_firstTabIndex;
@property(nonatomic) __weak TTTChannelViewContoller *channelVC; // @synthesize channelVC=_channelVC;
@property(retain, nonatomic) TTTTabConfigModel *tabConfig; // @synthesize tabConfig=_tabConfig;
@property(nonatomic) _Bool isTabSelect; // @synthesize isTabSelect=_isTabSelect;
@property(nonatomic) unsigned long long imageStatus; // @synthesize imageStatus=_imageStatus;
@property(nonatomic) double tabWidth; // @synthesize tabWidth=_tabWidth;
@property(copy, nonatomic) NSNumber *textWidth; // @synthesize textWidth=_textWidth;
@property(copy, nonatomic) NSNumber *imageWidth50; // @synthesize imageWidth50=_imageWidth50;
@property(copy, nonatomic) NSNumber *height; // @synthesize height=_height;
@property(copy, nonatomic) NSNumber *width; // @synthesize width=_width;
@property(nonatomic) long long tabType; // @synthesize tabType=_tabType;
@property(nonatomic) __weak NSString *tabStyle; // @synthesize tabStyle=_tabStyle;
@property(copy, nonatomic) NSString *floorTemplate; // @synthesize floorTemplate=_floorTemplate;
@property(nonatomic) double nameWidth; // @synthesize nameWidth=_nameWidth;
@property(nonatomic) struct CGPoint horizontalContentOffset; // @synthesize horizontalContentOffset=_horizontalContentOffset;
@property(retain, nonatomic) TTTMultiTabListModel *multiFloorModel; // @synthesize multiFloorModel=_multiFloorModel;
@property(copy, nonatomic) NSString *multiParams; // @synthesize multiParams=_multiParams;
@property(copy, nonatomic) NSString *bottomText; // @synthesize bottomText=_bottomText;
@property(copy, nonatomic) NSString *topText; // @synthesize topText=_topText;
@property(nonatomic) long long rushState; // @synthesize rushState=_rushState;
@property(retain, nonatomic) NSString *stageStatus; // @synthesize stageStatus=_stageStatus;
@property(copy, nonatomic) NSNumber *timeRemain; // @synthesize timeRemain=_timeRemain;
@property(nonatomic) unsigned long long selectSecondTabIndex; // @synthesize selectSecondTabIndex=_selectSecondTabIndex;
@property(retain, nonatomic) NSArray *secondTabList; // @synthesize secondTabList=_secondTabList;
@property(copy, nonatomic) NSString *activePicUrl; // @synthesize activePicUrl=_activePicUrl;
@property(copy, nonatomic) NSString *defaultPicUrl; // @synthesize defaultPicUrl=_defaultPicUrl;
@property(copy, nonatomic) NSString *backgroundColor; // @synthesize backgroundColor=_backgroundColor;
@property(copy, nonatomic) NSString *name; // @synthesize name=_name;
@property(copy, nonatomic) NSString *groupId; // @synthesize groupId=_groupId;
- (void).cxx_destruct;
- (_Bool)isShowSecondTabView;
- (id)commonJumpObject:(unsigned long long)arg1;
- (id)thirdJumpObject;
- (id)sencondJumpObject;
- (id)firstJumpObject;
- (id)secondTabModel;
- (void)convertMSecondToSecond;
- (void)setUpRushState;
- (void)keyValuesDidFinishConvertingToObject;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

