//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "SHVPSmallVideoLiveBaseModel.h"

#import "SHVPSmallVideoCelllProtocol-Protocol.h"
#import "SHVPSmallVideoJumpProtocol-Protocol.h"

@class NSString, SHVPSmallVideoJumpModel;

@interface SHVPSmallVideoLiveShopModel : SHVPSmallVideoLiveBaseModel <SHVPSmallVideoCelllProtocol, SHVPSmallVideoJumpProtocol>
{
    long long _shopId;
    NSString *_name;
    NSString *_img;
    NSString *_desc;
    NSString *_tag;
    long long _followed;
    SHVPSmallVideoJumpModel *_jump;
}

@property(retain, nonatomic) SHVPSmallVideoJumpModel *jump; // @synthesize jump=_jump;
@property(nonatomic) long long followed; // @synthesize followed=_followed;
@property(copy, nonatomic) NSString *tag; // @synthesize tag=_tag;
@property(copy, nonatomic) NSString *desc; // @synthesize desc=_desc;
@property(copy, nonatomic) NSString *img; // @synthesize img=_img;
@property(copy, nonatomic) NSString *name; // @synthesize name=_name;
@property(nonatomic) long long shopId; // @synthesize shopId=_shopId;
- (void).cxx_destruct;
- (id)sencondJumpObject;
- (id)firstJumpObject;
@property(readonly, nonatomic) double horizontalCellHeight;
@property(readonly, nonatomic) double cellHeight;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

