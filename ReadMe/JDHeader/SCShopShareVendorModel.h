//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSArray, NSString, UIImage;

@interface SCShopShareVendorModel : NSObject
{
    NSString *_shopName;
    UIImage *_vendorDisplayIcon;
    long long _vendorType;
    NSArray *_items;
}

@property(retain, nonatomic) NSArray *items; // @synthesize items=_items;
@property(nonatomic) long long vendorType; // @synthesize vendorType=_vendorType;
@property(copy, nonatomic) UIImage *vendorDisplayIcon; // @synthesize vendorDisplayIcon=_vendorDisplayIcon;
@property(copy, nonatomic) NSString *shopName; // @synthesize shopName=_shopName;
- (void).cxx_destruct;
@property(readonly, nonatomic) long long selectCount;
@property(readonly, nonatomic) _Bool selected;

@end

