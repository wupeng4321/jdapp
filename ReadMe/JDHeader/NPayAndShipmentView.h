//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "JDContentItem.h"

@class COCheckoutIntelligentDeliveryView, NSDictionary, NShipmentScheduleView, NShipmentView, OrderPayShipMapModel, UILabel, UIView;

@interface NPayAndShipmentView : JDContentItem
{
    _Bool _isIntelligentDelivery;
    NSDictionary *_entryDictionary;
    double _bottomLineMarginLeft;
    NShipmentScheduleView *_scheduleView;
    long long _style;
    NShipmentView *_shipmentView;
    OrderPayShipMapModel *_payShipMapModel;
    UILabel *_deliveryServiceLabel;
    UIView *_bottomSepartor;
    COCheckoutIntelligentDeliveryView *_intelligentDeliveryView;
}

+ (id)payAndShipmentViewWithStyle:(long long)arg1 model:(id)arg2;
@property(nonatomic) _Bool isIntelligentDelivery; // @synthesize isIntelligentDelivery=_isIntelligentDelivery;
@property(retain, nonatomic) COCheckoutIntelligentDeliveryView *intelligentDeliveryView; // @synthesize intelligentDeliveryView=_intelligentDeliveryView;
@property(retain, nonatomic) UIView *bottomSepartor; // @synthesize bottomSepartor=_bottomSepartor;
@property(retain, nonatomic) UILabel *deliveryServiceLabel; // @synthesize deliveryServiceLabel=_deliveryServiceLabel;
@property(retain, nonatomic) OrderPayShipMapModel *payShipMapModel; // @synthesize payShipMapModel=_payShipMapModel;
@property(retain, nonatomic) NShipmentView *shipmentView; // @synthesize shipmentView=_shipmentView;
@property(nonatomic) long long style; // @synthesize style=_style;
@property(readonly, nonatomic) NShipmentScheduleView *scheduleView; // @synthesize scheduleView=_scheduleView;
@property(nonatomic) double bottomLineMarginLeft; // @synthesize bottomLineMarginLeft=_bottomLineMarginLeft;
@property(retain, nonatomic) NSDictionary *entryDictionary; // @synthesize entryDictionary=_entryDictionary;
- (void).cxx_destruct;
- (void)setIntelligentDeliveryMessage:(id)arg1 andButtonMessage:(id)arg2 andToUseBlock:(CDUnknownBlockType)arg3;
- (long long)__convertNPayAndShipmentStyle:(long long)arg1;
- (void)__setupUIWithStyle:(long long)arg1;
- (double)addOtherViewWithY:(double)arg1;
- (void)setText:(id)arg1 payment:(id)arg2 shipment:(id)arg3;
@property(readonly, nonatomic) UILabel *entryLabel;
@property(readonly, nonatomic) UILabel *shipmentLabel;
@property(readonly, nonatomic) UILabel *paymentLabel;
@property(readonly, nonatomic) UILabel *textLabel;
@property(readonly, nonatomic) UIView *accessory;
- (void)touchesCancelled:(id)arg1 withEvent:(id)arg2;
- (void)touchesEnded:(id)arg1 withEvent:(id)arg2;
- (void)touchesBegan:(id)arg1 withEvent:(id)arg2;
- (void)setBotomLineFrame;
- (void)resetHeightOfView;
- (void)setBottomLineHidden:(_Bool)arg1;
- (void)layoutSubviews;
- (id)initWithFrame:(struct CGRect)arg1 style:(long long)arg2;
- (id)initWithFrame:(struct CGRect)arg1 style:(long long)arg2 model:(id)arg3;
- (id)initWithFrame:(struct CGRect)arg1;
- (id)init;
- (void)dealloc;

@end

