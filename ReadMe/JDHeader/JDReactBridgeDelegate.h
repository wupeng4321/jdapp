//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "RCTBridgeDelegate-Protocol.h"

@class NSString;

@interface JDReactBridgeDelegate : NSObject <RCTBridgeDelegate>
{
    _Bool _isFullBundle;
    NSString *_debugURL;
    NSString *_moduleName;
    NSString *_modulePath;
}

@property(retain, nonatomic) NSString *modulePath; // @synthesize modulePath=_modulePath;
@property(retain, nonatomic) NSString *moduleName; // @synthesize moduleName=_moduleName;
@property(nonatomic) _Bool isFullBundle; // @synthesize isFullBundle=_isFullBundle;
@property(retain, nonatomic) NSString *debugURL; // @synthesize debugURL=_debugURL;
- (void).cxx_destruct;
- (void)loadSourceForBridge:(id)arg1 withBlock:(CDUnknownBlockType)arg2;
- (id)sourceURLForBridge:(id)arg1;
- (id)initWithModule:(id)arg1 debugURL:(id)arg2;
- (id)initWithModule:(id)arg1;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

