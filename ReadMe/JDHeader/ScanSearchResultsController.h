//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class JDStoreNetwork, NSError, NSString, ScanSearchResults, UIImage;
@protocol ScanSearchResultsControllerDelegate;

@interface ScanSearchResultsController : NSObject
{
    id <ScanSearchResultsControllerDelegate> _delegate;
    ScanSearchResults *_results;
    NSError *_error;
    NSString *_isFirstQuery;
    NSString *_source;
    JDStoreNetwork *_network;
    UIImage *_image;
    struct CGRect _selectedBounds;
}

@property(nonatomic) struct CGRect selectedBounds; // @synthesize selectedBounds=_selectedBounds;
@property(retain, nonatomic) UIImage *image; // @synthesize image=_image;
@property(retain, nonatomic) JDStoreNetwork *network; // @synthesize network=_network;
@property(readonly, copy, nonatomic) NSString *source; // @synthesize source=_source;
@property(retain, nonatomic) NSString *isFirstQuery; // @synthesize isFirstQuery=_isFirstQuery;
@property(retain, nonatomic) NSError *error; // @synthesize error=_error;
@property(retain, nonatomic) ScanSearchResults *results; // @synthesize results=_results;
@property(nonatomic) __weak id <ScanSearchResultsControllerDelegate> delegate; // @synthesize delegate=_delegate;
- (void).cxx_destruct;
- (void)EventCountCameraPurchaseFilterTypeWith:(id)arg1 sexId:(id)arg2 cat3Index:(id)arg3 styleIndex:(id)arg4 brandIndex:(id)arg5;
- (void)click:(id)arg1;
- (void)exposure:(id)arg1;
- (void)fetchResultsWithMainCategoryId:(id)arg1 subCategoryId:(id)arg2 hotCategoryId:(id)arg3 sexId:(id)arg4 isFirstQuery:(id)arg5 triggerQueryType:(id)arg6 isUserRectangle:(id)arg7 cat3Id:(id)arg8 styleId:(id)arg9 brandId:(id)arg10 isAttrFilter:(id)arg11;
- (void)fetchResultsWithMainCategoryId:(id)arg1 subCategoryId:(id)arg2 hotCategoryId:(id)arg3 sexId:(id)arg4 isFirstQuery:(id)arg5 triggerQueryType:(id)arg6 isUserRectangle:(id)arg7;
- (void)modifyMainBodySelectedBounds:(struct CGRect)arg1 indexPath:(long long)arg2;
- (void)modifySelectedBounds:(struct CGRect)arg1;
- (void)selectBrandId:(id)arg1;
- (void)selectStyleId:(id)arg1;
- (void)selectCat3Id:(id)arg1;
- (void)selectSexId:(id)arg1;
- (void)selectHotCategoryId:(id)arg1;
- (void)selectSubCategoryId:(id)arg1;
- (void)selectMainCategoryId:(id)arg1;
- (void)fetch;
@property(readonly, nonatomic) UIImage *imageForSelectedBounds;
- (void)dealloc;
- (id)initWithSource:(id)arg1 image:(id)arg2 selectedBounds:(struct CGRect)arg3;

@end

