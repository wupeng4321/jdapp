//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIView.h>

@class JDActivityIndicatorView, JDFooterLoadingView, NSMutableArray, NSString, NewRefreshTableHeaderView, SHVPListDataProvider, SHVPListTabItem, UILabel, UITableView;
@protocol SHVPFinderListContentViewDelegate;

@interface SHVPFinderListContentView : UIView
{
    _Bool _isLoading;
    _Bool _canLoadMore;
    _Bool _isPageLoaded;
    _Bool _aTest;
    NSString *_innerAnchor;
    id <SHVPFinderListContentViewDelegate> _delegate;
    CDUnknownBlockType _firstLoadDataCall;
    SHVPListDataProvider *_dataProvider;
    UITableView *_tableView;
    NSMutableArray *_listArr;
    long long _requestStatus;
    NSString *_requestOffset;
    NewRefreshTableHeaderView *_refreshHeaderView;
    JDFooterLoadingView *_loadingView;
    UILabel *_footReloadView;
    UILabel *_footNoMoreItemView;
    JDActivityIndicatorView *_activityIndicatorView;
    SHVPListTabItem *_tabItem;
}

@property(nonatomic, getter=isATest) _Bool aTest; // @synthesize aTest=_aTest;
@property(retain, nonatomic) SHVPListTabItem *tabItem; // @synthesize tabItem=_tabItem;
@property(retain, nonatomic) JDActivityIndicatorView *activityIndicatorView; // @synthesize activityIndicatorView=_activityIndicatorView;
@property(retain, nonatomic) UILabel *footNoMoreItemView; // @synthesize footNoMoreItemView=_footNoMoreItemView;
@property(retain, nonatomic) UILabel *footReloadView; // @synthesize footReloadView=_footReloadView;
@property(retain, nonatomic) JDFooterLoadingView *loadingView; // @synthesize loadingView=_loadingView;
@property(retain, nonatomic) NewRefreshTableHeaderView *refreshHeaderView; // @synthesize refreshHeaderView=_refreshHeaderView;
@property(copy, nonatomic) NSString *requestOffset; // @synthesize requestOffset=_requestOffset;
@property(nonatomic) long long requestStatus; // @synthesize requestStatus=_requestStatus;
@property(nonatomic) _Bool isPageLoaded; // @synthesize isPageLoaded=_isPageLoaded;
@property(nonatomic) _Bool canLoadMore; // @synthesize canLoadMore=_canLoadMore;
@property(nonatomic) _Bool isLoading; // @synthesize isLoading=_isLoading;
@property(retain, nonatomic) NSMutableArray *listArr; // @synthesize listArr=_listArr;
@property(retain, nonatomic) UITableView *tableView; // @synthesize tableView=_tableView;
@property(retain, nonatomic) SHVPListDataProvider *dataProvider; // @synthesize dataProvider=_dataProvider;
@property(copy, nonatomic) CDUnknownBlockType firstLoadDataCall; // @synthesize firstLoadDataCall=_firstLoadDataCall;
@property(nonatomic) __weak id <SHVPFinderListContentViewDelegate> delegate; // @synthesize delegate=_delegate;
@property(copy, nonatomic) NSString *innerAnchor; // @synthesize innerAnchor=_innerAnchor;
- (void).cxx_destruct;
- (id)currentNavigationController;
- (void)retryLoadMore;
- (void)refreshData;
- (void)expoTableView:(id)arg1 willDisplayCell:(id)arg2 forRowAtIndexPath:(id)arg3;
- (void)tableView:(id)arg1 didEndDisplayingCell:(id)arg2 forRowAtIndexPath:(id)arg3;
- (void)tableView:(id)arg1 willDisplayCell:(id)arg2 forRowAtIndexPath:(id)arg3;
- (_Bool)isCellCompletelyVisable:(id)arg1;
- (void)checkVisibilityOfCell:(id)arg1;
- (void)scrollViewDidEndDragging:(id)arg1 willDecelerate:(_Bool)arg2;
- (void)scrollViewDidEndScrollingAnimation:(id)arg1;
- (void)scrollViewDidScroll:(id)arg1;
- (void)validateDataList;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (double)tableView:(id)arg1 heightForRowAtIndexPath:(id)arg2;
- (double)tableView:(id)arg1 heightForHeaderInSection:(long long)arg2;
- (id)tableView:(id)arg1 viewForHeaderInSection:(long long)arg2;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;
- (long long)numberOfSectionsInTableView:(id)arg1;
- (void)startLoadingAnimateCircle;
- (void)stopLoadingAnimateCircle;
- (void)fetchDataScrollUp;
- (void)fetchDataPullDown;
- (void)fetchData:(long long)arg1;
- (void)saveOffset:(struct CGPoint)arg1;
- (struct CGPoint)validateOffset:(struct CGPoint)arg1;
- (void)loadLiveListData;
- (void)liveListContentTurnOffScrollTop;
- (void)bindTabItemModel:(id)arg1;
- (void)setUpRefreshHeader;
- (void)refreshList;
- (void)loadDataFirstTime;
- (void)initData;
- (void)dealloc;
- (id)initWithFrame:(struct CGRect)arg1;

@end

