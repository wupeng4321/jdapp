//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "FinderTabBarItem.h"

@class FinderTabBarShareItemModel, UIImageView, UIView;

@interface FinderTabBarShareItem : FinderTabBarItem
{
    FinderTabBarShareItemModel *_model;
    UIView *_clickView;
    UIImageView *_shareImageView;
}

@property(retain, nonatomic) UIImageView *shareImageView; // @synthesize shareImageView=_shareImageView;
@property(retain, nonatomic) UIView *clickView; // @synthesize clickView=_clickView;
@property(retain, nonatomic) FinderTabBarShareItemModel *model; // @synthesize model=_model;
- (void).cxx_destruct;
- (void)clickBlockRecognizer:(id)arg1;
- (void)refreshContent;
- (void)assignData:(id)arg1;
- (id)getModel;
- (void)layoutSubviews;
- (void)setupViews;
- (id)initWithFrame:(struct CGRect)arg1;

@end

