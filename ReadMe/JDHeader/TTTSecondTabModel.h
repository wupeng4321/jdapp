//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "TTTBaseModel.h"

#import "TTTJumpProtocol-Protocol.h"

@class NSDictionary, NSString, TTTBuryDataModel, TTTPagedFloorModel, TTTTabConfigModel;

@interface TTTSecondTabModel : TTTBaseModel <TTTJumpProtocol>
{
    NSString *_groupId;
    NSString *_name;
    NSString *_advertId;
    NSString *_type;
    NSString *_secondTabId;
    NSString *_secondName;
    NSDictionary *_params;
    NSString *_feedParam;
    TTTTabConfigModel *_tabConfig;
    NSString *_floorTemplate;
    TTTPagedFloorModel *_pageFloors;
    long long _currentPageNum;
    long long _netStatus;
    TTTBuryDataModel *_buryData;
}

@property(retain, nonatomic) TTTBuryDataModel *buryData; // @synthesize buryData=_buryData;
@property(nonatomic) long long netStatus; // @synthesize netStatus=_netStatus;
@property(nonatomic) long long currentPageNum; // @synthesize currentPageNum=_currentPageNum;
@property(retain, nonatomic) TTTPagedFloorModel *pageFloors; // @synthesize pageFloors=_pageFloors;
@property(copy, nonatomic) NSString *floorTemplate; // @synthesize floorTemplate=_floorTemplate;
@property(retain, nonatomic) TTTTabConfigModel *tabConfig; // @synthesize tabConfig=_tabConfig;
@property(copy, nonatomic) NSString *feedParam; // @synthesize feedParam=_feedParam;
@property(retain, nonatomic) NSDictionary *params; // @synthesize params=_params;
@property(copy, nonatomic) NSString *secondName; // @synthesize secondName=_secondName;
@property(copy, nonatomic) NSString *secondTabId; // @synthesize secondTabId=_secondTabId;
@property(copy, nonatomic) NSString *type; // @synthesize type=_type;
@property(copy, nonatomic) NSString *advertId; // @synthesize advertId=_advertId;
@property(copy, nonatomic) NSString *name; // @synthesize name=_name;
@property(copy, nonatomic) NSString *groupId; // @synthesize groupId=_groupId;
- (void).cxx_destruct;
- (id)commonJumpObject:(unsigned long long)arg1;
- (id)firstJumpObject;
- (void)savePageFloors:(id)arg1;
- (void)keyValuesDidFinishConvertingToObject;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

